...
import type { Item } from '@/myJs/class/item/item'
import { _ITEM_USE_STATE, ITEM_BOX, type _ALLITEM_FUNC } from '@/myJs/class/item/static'
...
export class ItemArray {
    ...
    public addItem (item: Item) {
        ...
        // this.user.affectPlayground().pushText(randomEFA(ITEM_BOX[this.itemArray[0].getType()].state[this.itemArray[0].getState()]))
    }
    ...
    public removeItem (x: {uuid: string, state: _ITEM_USE_STATE}) {
        ...
        if (index >= 0) {
            ...
            //     user: this.user,
            //     state: _ITEM_USE_STATE.REMOVED
            // })
            // this.user.affectPlayground().pushText(randomEFA(ITEM_BOX[this.itemArray[0].getType()].state[removedItem.getState()]))
        } else {
